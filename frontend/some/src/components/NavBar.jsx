 ...
 useEffect(() => {
  const fetchUserProfile = async () => {
    try {
      ...
      console.log('Response:', data);  // Agrega esto para verificar la respuesta completa
      if (response.ok) {
        setUser(data.user);
      } else {
        setMessage(data.error || 'Failed to fetch user profile');
      }
    } catch (error) {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setMessage(data.error || 'Error: Logout failed');
      }
    } catch (e) {
                      ...
                      >
                        {userNavigation.map((item) => (
                          <MenuItem key={item.name}>
                            <a
                              href={item.href}
                              onClick={item.onClick} // Add onClick handler for logout
          ...
          <DisclosurePanel className="md:hidden">
            ...
            <div className="border-t border-gray-700 pb-3 pt-4">
              <div className="flex items-center px-5">
                <div className="flex-shrink-0">
                  <img alt="" src={user ? user.imageUrl : ''} className="h-10 w-10 rounded-full" />
                </div>
                <div className="ml-3">
